...
import qb.fragments.AudioConversationFragment;
import qb.fragments.BaseConversationFragment;
import qb.fragments.ConversationFragmentCallbackListener;
import qb.fragments.IncomeCallFragment;
import qb.fragments.IncomeCallFragmentCallbackListener;
import qb.fragments.VideoConversationFragment;
import qb.util.NetworkConnectionChecker;
import qb.utils.Consts;
import qb.utils.FragmentExecuotr;
import qb.utils.PermissionsChecker;
import qb.utils.RingtonePlayer;
import qb.utils.SettingsUtil;
...
public class CallActivity extends BaseActivity implements QBRTCClientSessionCallbacks, QBRTCSessionStateCallback<QBRTCSession>, QBRTCSignalingCallback,
        IncomeCallFragmentCallbackListener, ConversationFragmentCallbackListener, NetworkConnectionChecker.OnConnectivityChangedListener{
...
//    private SharedPreferences sharedPref;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int activity_main = getResources().getIdentifier("activity_call", "layout", getPackageName());
...
//        int preferences = getResources().getIdentifier("preferences", "xml", getPackageName());
...
//        sharedPref = PreferenceManager.getDefaultSharedPreferences(this);
        ...
        int beep = getResources().getIdentifier("beep", "xml", getPackageName());
        ringtonePlayer = new RingtonePlayer(this, beep);

        int connection_popup = getResources().getIdentifier("connection_popup", "layout", getPackageName());
        connectionView = (LinearLayout) View.inflate(this, connection_popup, null);
...
//                startScreenSharing(data);
    ...
    private void initAudioManager() {
        ...
        audioManager.setOnWiredHeadsetStateListener((plugged, hasMicrophone) -> {
            if (callStarted) {
                logMe("setOnWiredHeadsetStateListener: Headset " + (plugged ? "plugged" : "unplugged"));
//                Toaster.shortToast("Headset " + (plugged ? "plugged" : "unplugged"));
        ...
        audioManager.setBluetoothAudioDeviceStateListener(connected -> {
            if (callStarted) {
                logMe("setBluetoothAudioDeviceStateListener: Bluetooth " + (connected ? "connected" : "disconnected"));
...
//        SettingsUtil.setSettingsStrategy(opponentsIdsList, sharedPref, CallActivity.this);
    ...
    private void setExpirationReconnectionTime() {
        int pref_disconnect_time_interval_key =
                getResources().getIdentifier("pref_disconnect_time_interval_key", "string", getPackageName());
        int pref_disconnect_time_interval_default_value =
                getResources().getIdentifier("pref_disconnect_time_interval_default_value", "string", getPackageName());
//        reconnectHangUpTimeMillis = SettingsUtil.getPreferenceInt(sharedPref, this, pref_disconnect_time_interval_key,
//                pref_disconnect_time_interval_default_value) * 1000;
        reconnectHangUpTimeMillis = 10000;
    ...
    private void showNotificationPopUp(final int text, final boolean show) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (show) {
                    int notification = getResources().getIdentifier("notification", "id", getPackageName());
                    ((TextView) connectionView.findViewById(notification)).setText(text);
                    if (connectionView.getParent() == null) {
                        int fragment_container = getResources().getIdentifier("fragment_container", "id", getPackageName());
                        ((ViewGroup) CallActivity.this.findViewById(fragment_container)).addView(connectionView);
                    }
                } else {
                    int fragment_container = getResources().getIdentifier("fragment_container", "id", getPackageName());
                    ((ViewGroup) CallActivity.this.findViewById(fragment_container)).removeView(connectionView);
                }
            }
        });
    ...
    private void initIncomingCallTask() {
        ...
        showIncomingCallWindowTask = new Runnable() {
            ...
            public void run() {
                ...
                logMe("Call was stopped by timer");
//                Toaster.longToast("Call was stopped by timer");
    ...
    private void showToast(final int message) {
        logMe("showToast:" + getString(message));
        /*
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
    ...
    public void onReceiveHangUpFromUser(final QBRTCSession session, final Integer userID, Map<String, String> map) {
        if (session.equals(getCurrentSession())) {
            ...
            int text_status_hang_up = getResources().getIdentifier("text_status_hang_up", "string", getPackageName());
            logMe("User " + participantName + " " + getString(text_status_hang_up) + " conversation");
//            showToast("User " + participantName + " " + getString(text_status_hang_up) + " conversation");
        }
    }
    ...
    private void addIncomeCallFragment() {
        ...
        if (currentSession != null) {
            ...
            int fragment_container = getResources().getIdentifier("fragment_container", "id", getPackageName());
            FragmentExecuotr.addFragment(getSupportFragmentManager(), fragment_container, fragment, INCOME_CALL_FRAGMENT);
        } else {
    ...
    private void addConversationFragment(boolean isIncomingCall) {
        BaseConversationFragment conversationFragment = BaseConversationFragment.newInstance(
                ...
                isIncomingCall);
        int fragment_container = getResources().getIdentifier("fragment_container", "id", getPackageName());
        FragmentExecuotr.addFragment(getSupportFragmentManager(), fragment_container, conversationFragment, conversationFragment.getClass().getSimpleName());
    }
...
//        return sharedPref;
    ...
    public void onErrorSendingPacket(QBSignalingSpec.QBSignalCMD qbSignalCMD, Integer userId, QBRTCSignalException e) {
        int dlg_signal_error = getResources().getIdentifier("fragment_container", "string", getPackageName());
        showToast(dlg_signal_error);
    }
...
*/
    ...
    private class ConnectionListener extends AbstractConnectionListener {
        ...
        public void connectionClosedOnError(Exception e) {
            int connection_was_lost = getResources().getIdentifier("connection_was_lost", "string", getPackageName());
            showNotificationPopUp(connection_was_lost, true);
        ...
        public void reconnectionSuccessful() {
            int connection_was_lost = getResources().getIdentifier("connection_was_lost", "string", getPackageName());
            showNotificationPopUp(connection_was_lost, false);
        }
