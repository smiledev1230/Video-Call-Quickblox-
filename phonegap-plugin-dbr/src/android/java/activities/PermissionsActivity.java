...
public class PermissionsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int activity_permissions = getResources().getIdentifier("activity_permissions", "layout", getPackageName());
        setContentView( activity_permissions );
    ...
    private void showDeniedResponse(int[] grantResults) {
        if (grantResults.length > 1) {
            for (int i = 0; i < grantResults.length; i++) {
                if (grantResults[i] != 0) {
                    int permission_unavailable = getResources().getIdentifier("permission_unavailable", "string", getPackageName());
                    Log.d("PermissionActivity", getString(permission_unavailable, permissionFeatures.values()[i]));
//                    Toaster.longToast(getString(R.string.permission_unavailable, permissionFeatures.values()[i]));
                }
            }
        } else {
            int permission_unavailable = getResources().getIdentifier("permission_unavailable", "string", getPackageName());
            Log.d("PermissionActivity", getString(permission_unavailable, permissionFeatures.MICROPHONE));
