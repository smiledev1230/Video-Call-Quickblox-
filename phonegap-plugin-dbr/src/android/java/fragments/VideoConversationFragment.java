...
import qb.adapters.OpponentsFromCallAdapter;
...
public class VideoConversationFragment extends BaseConversationFragment implements Serializable, QBRTCClientVideoTracksCallbacks<QBRTCSession>,
    ...
    protected void configureOutgoingScreen() {
        outgoingOpponentsRelativeLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), getColorIdentifier("grey_transparent_50")));
        allOpponentsTextView.setTextColor(ContextCompat.getColor(getActivity(), getColorIdentifier("white")));
        ringingTextView.setTextColor(ContextCompat.getColor(getActivity(), getColorIdentifier("white")));
    }
    ...
    protected void configureToolbar() {
        ...
        toolbar.setBackgroundColor(ContextCompat.getColor(getActivity(), getColorIdentifier("black_transparent_50")));
        toolbar.setTitleTextColor(ContextCompat.getColor(getActivity(), getColorIdentifier("white")));
        toolbar.setSubtitleTextColor(ContextCompat.getColor(getActivity(), getColorIdentifier("white")));
    }

    @Override
    int getFragmentLayout() {
        return getResources().getIdentifier("fragment_video_conversation", "layout", getActivity().getPackageName());
    }
    ...
    protected void initFields() {
        ...
        timerChronometer = (Chronometer) getActivity().findViewById(getIdIdentifier("timer_chronometer_action_bar"));
    ...
    public void setDuringCallActionBar() {
        ...
        if (isPeerToPeerCall) {
            actionBar.setSubtitle(getString(getStringIdentifier("opponent"), opponents.get(0).getFullName()));
        } else {
            actionBar.setSubtitle(getString(getStringIdentifier("opponents"), amountOpponents));
        }
    ...
    protected void initViews(View view) {
        ...
        localVideoView = (QBRTCSurfaceView) view.findViewById(getIdIdentifier("local_video_view"));
        initCorrectSizeForLocalView();
        localVideoView.setZOrderMediaOverlay(true);

        remoteFullScreenVideoView = (QBRTCSurfaceView) view.findViewById(getIdIdentifier("remote_video_view"));
        remoteFullScreenVideoView.setOnClickListener(localViewOnClickListener);

        if (!isPeerToPeerCall) {
            recyclerView = (RecyclerView) view.findViewById(getIdIdentifier("grid_opponents"));

            recyclerView.addItemDecoration(new DividerItemDecoration(getActivity(), getDimenIdentifier("grid_item_divider")));
...
//          for correct removing item in adapter
        ...
        connectionStatusLocal = (TextView) view.findViewById(getIdIdentifier("connectionStatusLocal"));

        cameraToggle = (ToggleButton) view.findViewById(getIdIdentifier("toggle_camera"));
        cameraToggle.setVisibility(View.VISIBLE);

        actionVideoButtonsLayout = (LinearLayout) view.findViewById(getIdIdentifier("element_set_video_buttons"));
    ...
    private void setGrid(int columnsCount) {
        ...
        float itemMargin = getResources().getDimension(getDimenIdentifier("grid_item_divider"));
        ...
        opponentsAdapter = new OpponentsFromCallAdapter(getActivity(), currentSession, opponents, cellSizeWidth,
                (int) getResources().getDimension(getDimenIdentifier("item_height")));
...
//      get mainVideoTrack - opponent's video track from full screen
    ...
    private void setRecyclerViewVisibleState() {
        ...
        params.height = (int) getResources().getDimension(getDimenIdentifier("item_height"));
    ...
    private void updateNameForOpponent(int userId, String newUserName) {
        if (isPeerToPeerCall) {
            actionBar.setSubtitle(getString(getStringIdentifier("opponent"), newUserName));
        } else {
    ...
    public void onConnectedToUser(QBRTCSession qbrtcSession, final Integer userId) {
        ...
        setStatusForOpponent(userId, getString(getStringIdentifier("text_status_connected")));
        setProgressBarForOpponentGone(userId);
    }

    @Override
    public void onConnectionClosedForUser(QBRTCSession qbrtcSession, Integer userId) {
        setStatusForOpponent(userId, getString(getStringIdentifier("text_status_closed")));
        if (!isPeerToPeerCall) {
    ...
    public void onDisconnectedFromUser(QBRTCSession qbrtcSession, Integer integer) {
        setStatusForOpponent(integer, getString(getStringIdentifier("text_status_disconnected")));
    }
    ...
    public void onUserNotAnswer(QBRTCSession session, Integer userId) {
        setProgressBarForOpponentGone(userId);
        setStatusForOpponent(userId, getString(getStringIdentifier("text_status_no_answer")));
    }

    @Override
    public void onCallRejectByUser(QBRTCSession session, Integer userId, Map<String, String> userInfo) {
        setStatusForOpponent(userId, getString(getStringIdentifier("text_status_rejected")));
    }

    @Override
    public void onCallAcceptByUser(QBRTCSession session, Integer userId, Map<String, String> userInfo) {
        setStatusForOpponent(userId, getString(getStringIdentifier("accepted")));
    }

    @Override
    public void onReceiveHangUpFromUser(QBRTCSession session, Integer userId, Map<String, String> userInfo) {
        setStatusForOpponent(userId, getString(getStringIdentifier("text_status_hang_up")));
...
//      get opponentVideoTrack - opponent's video track from recyclerView
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        // int conversation_fragment = getResources().getIdentifier("conversation_fragment", "menu", getActivity().getPackageName());
        // inflater.inflate(conversation_fragment, menu);
...
//                return true;
    ...
    class LocalViewOnClickListener implements View.OnClickListener {
        ...
        private void shiftMarginListOpponents() {
            ...
            params.setMargins(0, 0, 0, (int) getResources().getDimension(getDimenIdentifier("margin_common")));
    ...
    public int getStringIdentifier(String res){
        return getResources().getIdentifier(res, "string", getActivity().getPackageName());
    }
    ...
    public int getDimenIdentifier(String res){
        return getResources().getIdentifier(res, "dimen", getActivity().getPackageName());
    }
